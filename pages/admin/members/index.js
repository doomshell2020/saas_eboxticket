...
const MembersTable = ({ initialMemberData }) => {

  const [DATATABLE, SetDATATABLE] = useState(initialMemberData || []);

  useEffect(() => {
    SetDATATABLE(initialMemberData || []);
  }, [initialMemberData]);
  ...
  // ✅ 1. Set form/filter states from URL query
  useEffect(() => {
    setPageSize(50);
    ...
    setInterests(
      query.member_interest
        ? query.member_interest.split(",").map((value) =>
    ...
    setCountry(
      query.member_location
        ? query.member_location.split(",").map((value) =>
  ...
  // ✅ 2. Conditionally fetch data if not SSR
  useEffect(() => {
    if (hasFetched.current) {
      // fetchData(query);
    } else {
      hasFetched.current = true; // skip first time (SSR)
    }
  }, [
    query.fname,
    query.lname,
  ...
  const routeChange = () => {
    router.push({
      pathname: "/admin/members", // or your current path
...
export async function getServerSideProps(context) {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/v2/members/getAllMembers?${queryParams.toString()}`);
    const data = await res.json();
    // console.log('>>>>>>>>>>',queryParams);    

    return {
      props: {
        initialMemberData: data?.data || [],
      },
    };
  } catch (error) {
