...
const EventEdit = ({ currencies, countries, eventData, statesList }) => {
  ...
  const [preview, setPreview] = useState(null); // if editing, show existing image
  ...
  useEffect(() => {
    if (event && event.id) {
      ...
      if (event?.ImageURL) {
        setPreview(`${baseUrl}uploads/profiles/${event?.ImageURL}`)
      }
  ...
  const EventsUpdate = async (event) => {
    ...
    if (form.checkValidity() == false) {
      ...
      const EventsEditUrl = `/api/v1/events?id=${eventid}`;
      event.preventDefault();
      const body = new FormData();
      body.append("Name", Name);

      if (ImageURL) {
        body.append("eventImage", ImageURL);        
      }
      ...
      await axios.put(EventsEditUrl, body)
        .then((res) => {
          console.log('>>>>>>>>', res.data.EdiEvents.success);

          if (res.data.EdiEvents.success) {
            setIsLoading(false);
            Swal.fire({
              icon: "success",
              title: `${Name} Event Updated!`, // Dynamic event name in title
              text: `The event "${Name}" has been successfully updated!`, // Clear update message
              confirmButtonText: "Awesome! 🎉", // More engaging confirmation button text
            ...
            Swal.fire({
              icon: "error",
              title: "Update Failed!",
              text: res.data.error || "Unable to update the event. Please try again.",
            ...
            <Card.Body className="p-sm-3 p-2">
              ...
              <div className="add-event-progress-bar">
                <ul className="progress-br-head mx-auto">
                  ...
                  <li className="active">
                    <Link href={`/admin/events/edit/${eventid ? `${eventid}` : "#"}`}>
                  ...
                  <li>
                    <Link
                      href={eventid ? `/admin/events/edit/ticket-types/${eventid}` : "#"}
                      style={{
                        pointerEvents: eventid ? "auto" : "none",
                        color: eventid ? "inherit" : "gray",
                      }}
                    >
                      <span>Tickets</span>
                    </Link>
                  ...
                  <li>
                    <Link
                      href={eventid ? `/admin/events/edit/publish-event/${eventid}` : "#"}
                      style={{
                        pointerEvents: eventid ? "auto" : "none",
                        color: eventid ? "inherit" : "gray",
                      }}
              ...
              >
                ...
                <CCol lg={3} md={6}>
                  ...
                  <CFormInput
                    type="file"
                    id="eventImageInput"
                    required={!eventid} // Required only on create
                    accept=".png, .jpg, .jpeg"
                ...
                <Modal show={showModal} onHide={() => setShowModal(false)} centered>
                  <Modal.Header closeButton>
                    <Modal.Title>Event Image Preview</Modal.Title>
                  </Modal.Header>
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    const [currencyRes, countryRes, eventRes] = await Promise.all([
      ...
      axios.get(`${baseUrl}/api/v1/events?id=${eventid}`)
    ]);
