...
const HousingAvailability = ({ houseData, eventData, neighborhoodsData, housingTypesData }) => {
  const router = useRouter();
  const { eventId } = router.query;
  const [data, setHouseData] = useState(houseData || []);
  const [eventDetails, setEventDetails] = useState(eventData || {});
  const [neighborhoods, setNeighborhoods] = useState(neighborhoodsData || []);
  const [loading, setLoading] = useState(true);
  const [housingTypes, setHousingTypes] = useState(housingTypesData || []);
  const [isOpen, setIsOpen] = useState(false);
  useEffect(() => {
  // ✅ Initialize state only when props change
  setHouseData(houseData);
  setEventDetails(eventData);
  setNeighborhoods(housingTypesData);
  setLoading(false); // ✅ Set loading false after all data is set
}, [houseData, eventData, housingTypesData]);
  // searching states......
...
// Route Change
  const routeChange = () => {
    router.push({
      pathname: `/admin/events/housing-availability-new/${eventId}`, // or your current path
              ...
              <Card.Body className="">
                ...
                >
                  <DataTable
                    ...
                    paginationRowsPerPageOptions={[10, 25, 50, 100]} // ✅ Add dropdown
                  />
                </DataTableExtensions>
              </Card.Body>
            </Card>
...
//   const { eventId } = context.params || {}; // from dynamic route [eventId]
//   const baseUrl = process.env.SITE_URL || 'http://localhost:3000';

//   try {
//     // 1. Get Housings
//     const housingRes = await fetch(`${baseUrl}/api/v1/housings`);
//     const housingJson = await housingRes.json();
//     const houseData = housingJson?.data || [];

//     // 2. Get Event Details
//     const eventRes = await fetch(`${baseUrl}/api/v1/events/?id=${eventId}`);
//     const eventJson = await eventRes.json();
//     const eventData = eventJson?.data || null;

//     // 3. Get Neighborhoods
//     const neighborhoodRes = await fetch(`${baseUrl}/api/v1/housings-new/?key=getHousingNeighborhood`);
//     const neighborhoodJson = await neighborhoodRes.json();
//     const neighborhoodsData = neighborhoodJson?.data || [];

//     // 4. Get Housing Types
//     const housingTypesRes = await fetch(`${baseUrl}/api/v1/housings-new/?key=get_housingTypes`);

//     const housingTypesJson = await housingTypesRes.json();
//     const housingTypesData = housingTypesJson?.data || [];
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    // ✅ 1. If search is triggered, use query string
    if (query.key == 'search_event_housing') {
      const searchParams = new URLSearchParams(query).toString();
      const searchRes = await fetch(`${baseUrl}/api/v1/housings?${searchParams}`);
      const searchJson = await searchRes.json();
      houseData = searchJson?.data || [];
    } else {
      // ✅ 2. Default Housing list
      const housingRes = await fetch(`${baseUrl}/api/v1/housings`);
      const housingJson = await housingRes.json();
      houseData = housingJson?.data || [];
    }
    ...
    return {
      props: {
        ...
        neighborhoodsData,
        housingTypesData,
        isSearch: query.key === 'search_event_housing',
      },
    };
  } catch (error) {
    console.error('SSR Error:', error);
