...
import { Breadcrumb, Card, Col, Spinner, Row } from "react-bootstrap";
...
const CareyeshousingEdit = () => {
    ...
    const [termsAndConditions, setTermsAndConditions] = useState("");  //t&c section
    ...
    const transformHousingsData = (housings) => {
        const groupedBedrooms = {};

        // Group beds by bedroom_number
        housings.forEach((bed) => {
            if (!groupedBedrooms[bed.bedroom_number]) {
                groupedBedrooms[bed.bedroom_number] = {
                    bedroom_number: bed.bedroom_number,
                    beds: [],
                };
            }
            groupedBedrooms[bed.bedroom_number].beds.push({
                bed_number: bed.bed_number,
                bed_type: bed.bed_type,
            });
        });
        // Convert the grouped object into an array
        return Object.values(groupedBedrooms);
    };


    useEffect(() => {
        if (housingId != undefined) {
            var housingViewUrl = `/api/v1/housings-new?housing_id=${housingId}`
            fetch(housingViewUrl)
                .then((response) => response.json())
                .then((value) => {
                    // console.log(value.data)
                    setName(value.data.Name);
                    setNeighborhood(value.data.Neighborhood);
                    setType(value.data.Type);
                    setNumBedrooms(value.data.NumBedrooms)
                    setPool(value.data.Pool)
                    setMaxOccupancy(value.data.MaxOccupancy);
                    setManagerEmail(value.data.ManagerEmail);
                    setManagerName(value.data.ManagerName);
                    setManagerMobile(value.data.ManagerMobile);
                    setOwnerName(value.data.OwnerName);
                    setOwnerEmail(value.data.OwnerEmail);
                    setOwnerMobile(value.data.OwnerMobile);
                    setLocation(value.data.location);
                    setImageURL(value.data.ImageURL); // Current image URL
                    setGoogleMap(value.data.google_map)
                    setDescription(value.data.Description);
                    setBookingStatus(value.data.bookingStatus);
                    setTermsAndConditions(value.data.terms_and_conditions);
                    //  setAmenities(value.data.amenities)
                    var Amenities = value.data.amenities;
                    const AmenitiesData = Amenities ? Amenities.split(",").map(Number) : [];
                    setSelectedAmenities(AmenitiesData);
                    setAdminNotes(value.data.admin_notes)
                    setBooking_notes(value.data.booking_notes)

                    if (value.data && value.data.Housings) {
                        // Transform the data
                        const transformedBedrooms = transformHousingsData(value.data.Housings);
                        setBedrooms(transformedBedrooms); // Set the transformed data
                    }
                    setTimeout(() => {
    ...
    const handleNumBedroomsChange = (e) => {
        ...
        const newBedrooms = [];
        for (let i = 1; i <= num; i++) {
            // Check if the bedroom already exists in the state
            const existingBedroom = bedrooms.find((bedroom) => bedroom.bedroom_number === i);
            if (existingBedroom) {
                // If it exists, retain its data
                newBedrooms.push(existingBedroom);
            } else {
                // If it doesn't exist, create a new bedroom with default data
                newBedrooms.push({
                    bedroom_number: i,
    ...
    const handleNumBedsChange = (bedroomIndex, numBeds) => {
        ...
        if (numBeds > beds.length) {
            for (let i = beds.length; i < numBeds; i++) {
                // Add a new bed with default data
                beds.push({ bed_number: i + 1, bed_type: "2" });
            }
        } else {
            // Remove extra beds, but keep the existing data for the remaining beds
    ...
    const handleSubmit = async (event) => {
        ...
        // const amenitiesContent = amenitiesRef.current.summernote('code');
        ...
        if (form.checkValidity() == false || error == "Invalid email format!" || emailError == "Invalid email format!") {
            ...
            // const housingAddUrl = `/api/v1/housings-new?id=${housingId}`;
            const housingAddUrl = `/api/v2/housings-new?id=${housingId}`;
            event.preventDefault();
            const body = new FormData();
            ...
            // body.append("amenities", amenitiesContent.trim());
            ...
            try {
                ...
                const res = await axios.put(housingAddUrl, body);
                if (res.data.success) {
                    localStorage.setItem("staticAdded", res.data.message);
                    routeChange();
                } else {
                    console.error("Server responded with failure:", message);
        ...
        <div>
            ...
            <div className="row">
                ...
                <Col lg={12} md={12}>
                    <Card>
                        <Card.Header>
                            <h3 className="card-title">Edit House</h3>
                        </Card.Header>
                        <Card.Body>
                            <CForm
                            ...
                            >
...

                                        {
                                            isClient ?
                                                <div >
                                                    <SummernoteLite
                                                        ref={bookNowRef}
                                                        defaultCodeValue={booking_notes}
                                ...
                                {/* <CCol md={3}>
                                    <CFormLabel htmlFor="validationDefault01">Owner Email</CFormLabel>
                                    <CFormInput
                                        type="email"
                                        id="validationDefault01"
                                ...
                                <CCol md={3}>
                                    ...
                                    {/* If a new image is selected, preview it; otherwise, show the current image */}
                                    {selectedImage ? (
                                        <Image
                                        ...
                                        <Image
                                            src={`${process.env.NEXT_PUBLIC_S3_URL}/housing/${ImageURL}`} // ✅ S3 hosted existing image
                                            alt="Event Image"
                                            style={{ marginTop: "10px", borderRadius: "10px" }}
                                            width={100}
                                            height={100}
                                        ...
                                        <Image
                                            src={`${process.env.NEXT_PUBLIC_S3_URL}/housing/housingdumy.png`} // ✅ Fallback from S3
                                            alt="Image Not Found"
                                            style={{ marginTop: "10px", borderRadius: "10px" }}
                                            width={100}
                                            height={100}
                                        ...
                                        isClient ?
                                            <div >
                                                <SummernoteLite
                                                    ...
                                                    // onChange={(content) => setData({ ...data, content })}
                                                    onChange={(content) => {
                                                        // Check if the content is empty or contains only the default <p><br></p>
                                                        if (content === '<p><br></p>' || content.trim() === '') {
                                                            setData({ ...data, content: null }); // or set to an empty string ''
                                                        } else {
                                        ...
                                        isClient ?
                                            <div >
                                                <SummernoteLite
                                                    ...
                                                    defaultCodeValue={termsAndConditions}
                                                    placeholder={"Write something here..."}
                                                    tabsize={2}
                                                    lang="zh-CN" // only if you want to change the default language
                                                    height={100 || "50vh"}
                                ...
                                {/* <CCol md={12}>
                                    <CFormLabel htmlFor="validationDefault01">Amenities</CFormLabel>
                                    {
                                        isClient ?
                                            <div >
                                                <SummernoteLite
                                                    ref={amenitiesRef}
                                                    defaultCodeValue={amenities}
                                                    placeholder={"Write something here..."}
                                                    tabsize={2}
                                                    lang="zh-CN" // only if you want to change the default language
                                                    height={100 || "50vh"}
                                ...
                                <CCol md={12}>
                                    <CFormLabel style={{ color: "#38cab3" }}><strong>ADMIN NOTES</strong></CFormLabel>
                                    {
                                        isClient ?
                                            <div >
                                                <SummernoteLite
                                                    ref={adminNotesRef}
                                                    defaultCodeValue={adminNotes}
                                                    placeholder={"Write something here..."}
                                                    tabsize={2}
                                                    lang="zh-CN" // only if you want to change the default language
                                                    height={100 || "50vh"}
                                ...
                                <CCol md={2} xs={6}>
                                    {/* <CButton color="info me-2" type="submit">
                                        Submit
                                    </CButton> */}
...
CareyeshousingEdit.propTypes = {};

CareyeshousingEdit.defaultProps = {};

CareyeshousingEdit.layout = "Contentlayout"

export default CareyeshousingEdit;
