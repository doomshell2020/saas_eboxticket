...
const webhookSecret = process.env.STRIPE_DEV_WEBHOOK_SECRET;
...
const handler = async (req, res) => {
  ...
  switch (event.type) {
    case "payment_intent.created":
      ...
      console.log(">>>>>>>>>>> testing-webhook-file Payment intent created : " + paymentIntentId);
      break;
    case "payment_intent.succeeded":
      ...
      // console.log('------------------testing-webhook-file  stripe webhook metadata--------------------', metadata);
      // console.log('------------------testing-webhook-file  stripe webhook metadata--------------------', rawPropertyDetailsObj);
      ...
      if (!userId || !eventId) {
        console.log("❌testing-webhook-file Missing required metadata fields: userId or eventId.");
        return res.status(400).json({
          success: false,
          message: "testing-webhook-file Missing required metadata fields: userId or eventId.",
        });
      }
      ...
      try {
        ...
        console.error("❌testing-webhook-file Invalid cartData format");
        return res.status(400).json({
          success: false,
          message: "testing-webhook-file Invalid cartData format. It should be a JSON array.",
        });
      }
      ...
      try {
        ...
        console.warn("⚠️ testing-webhook-file Failed to parse propertyDetailsObj, setting to null.");
      ...
      // console.log('✅ testing-webhook-file isPropertyObjValid:', isPropertyObjValid);
      // console.log('✅ testing-webhook-file isCartDataValid:', isCartDataValid);
      ...
      if (!isDuePayment && !isCartDataValid && !isPropertyObjValid) {
        return res.status(400).json({
          success: false,
          message: "testing-webhook-file Either cartData or propertyDetailsObj must have valid data.",
        });
      }
      ...
      const response = await axios.post(
        ...
        console.log("✅ testing-webhook-file Order created successfully.");
      } catch (err) {
        console.error("❌ testing-webhook-file Error creating order:", err.message);
    ...
    case "payment_intent.payment_failed":
      ...
      console.log(`testing-webhook-file PaymentIntent failed: ${paymentFailedIntent.id}`);
      break;
    default:
      console.log(`testing-webhook-file Unhandled event type ${event.type}`);
  }
