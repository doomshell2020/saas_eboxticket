...
export default function Cart({
  ...
  const calculateTotalsV1 = (
    ...
    cart.forEach((item) => {
      ...
      if (item.ticket_type == "ticket") {
        totalTicketPrice += price * item.no_tickets;
      } else {
        totalAddonPrice += price * item.no_tickets;
      }
    });
    ...
    // console.log('>>>>>>>>>>>>>>>>>>>>>', totalAfterTaxes);

    // const totalAccommodationPrice = propertyDetails ? nightlyRate * nights : 0;
    // const totalAccommodationBasePrice = propertyDetails ? basePriceHousing * nights : 0;

    // Calculate totals without rounding during multiplication
    const totalAccommodationPrice = propertyDetails
      ? Math.round(nightlyRate * nights)
      : 0;

    // const totalAccommodationBasePrice = propertyDetails
    //   ? Math.round(basePriceHousing * nights)
    //   : 0;
    const totalAccommodationBasePrice = propertyDetails
      ? Math.round(totalAfterTaxes * nights)
      : 0;

    // Total before discount (apply discount ONLY to ticket + addon)
    const totalBeforeDiscount = totalTicketAndAddonPrice + totalAccommodationPrice;

    // Apply discount only to tickets and addons
    const discountedTicketAndAddon = Math.max(
      totalTicketAndAddonPrice - discountAmount,
      0
    ); // Avoid negative

    // Proportional discount allocation (just for tax accuracy)
    const ticketRatio = totalTicketPrice / totalTicketAndAddonPrice || 0;
    const addonRatio = totalAddonPrice / totalTicketAndAddonPrice || 0;

    const discountedTicketTotal = totalTicketPrice - discountAmount * ticketRatio;
    const discountedAddonTotal = totalAddonPrice - discountAmount * addonRatio;
    const discountedTotalTicketAddonAccommodation = discountedTicketTotal + discountedAddonTotal + totalAccommodationBasePrice;
    const discountedTotalTicketAddon = discountedTicketTotal + discountedAddonTotal;

    // Accommodation is not discounted
    const discountedAccommodationTotal = totalAccommodationPrice;

    // Taxes
    const ticketTax = round2(discountedTicketTotal * (adminFeePercent / 100));
    const addonTax = round2(discountedAddonTotal * (adminFeePercent / 100));
    const accommodationTax = round2(totalAccommodationBasePrice * (accommodationTaxPercentage / 100));
    const ticketAddonTaxes = Math.round(ticketTax + addonTax);

    const totalTax = Math.round(ticketTax + addonTax + accommodationTax);

    const exactFinalAmount = discountedTicketAndAddon + totalAccommodationPrice + (totalTax - accommodationTax);
    const finalTotalAmount = Math.round(exactFinalAmount);

    // Partial payment handling
    const isPartial = selectedPaymentOption == "partial";

    const halfAccommodation = totalAccommodationPrice / 2;
    const halfAccommodationTax = accommodationTax / 2;

    const partialPayableAmount = Math.round(
      discountedTicketAndAddon + ticketTax + addonTax + halfAccommodation
    );

    const partialPayableTax = Math.round(
      ticketTax + addonTax + halfAccommodationTax
    );
    const payableAmount = isPartial ? partialPayableAmount : finalTotalAmount;

    // Destructure ticket fee percentages
    const {
    ...
    // Apply round2 ONLY here for ticket tax breakdown
    const ticketPlatformFee = round2(
      discountedTotalTicketAddon * (ticket_platform_fee_percentage / 100)
    );

    const ticketStripeFee = round2(
      discountedTotalTicketAddon *
      (ticket_stripe_fee_percentage / 100)
    );
    const ticketBankFee = round2(
      discountedTotalTicketAddon *
      (ticket_bank_fee_percentage / 100)
    );
    const ticketProcessingFee = round2(
      discountedTotalTicketAddon *
      (ticket_processing_fee_percentage / 100)
    );

    const accommodationBankFee = round2(
      totalAccommodationBasePrice * (ticket_bank_fee_percentage / 100)
    );
    const accommodationStripeFee = round2(
      totalAccommodationBasePrice * (ticket_stripe_fee_percentage / 100)
    );
    const accommodationProcessingFee = round2(
      totalAccommodationBasePrice * (ticket_processing_fee_percentage / 100)
    );

    // console.log('>>>>>>>>>', totalAccommodationBasePrice);
    ...
    return {
      breakdown: {
        ...
        totalBeforeDiscount,
        discountAmount,
        totalAfterDiscount: discountedTicketAndAddon + totalAccommodationPrice,
        ticketTax,
        addonTax,
        accommodationTax,
        totalTax,
  ...
  const { breakdown } = calculateTotalsV1(
    cart,
                                ...
                                (() => {
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      >
                                        {currencySymbol || ""}
                                        {formatSmartPrice(accommodationTotal)}
                                        {/* <img
                                ...
                                return (
                                  <div className="tickt-ttl-prs">
                                    ...
                                    {ticketAddonTaxes > 0 && (
                                      <div className="tct-ttl-innr">
                                        <p>
                                          TICKETS FEES & TAXES ({adminFees}%)
                                        </p>
                                        <span>
                                          {currencySymbol || ""}
                                          {formatSmartPrice(ticketAddonTaxes)}
                                        </span>
                                      </div>
                                    )}
                                    ...
                                    <div className="tct-ttl-innr">
                                      ...
                                      <p>
                                        {currencySymbol || ""}
                                        {formatSmartPrice(finalTotalAmount)}
                                      </p>
                                    </div>
                                  </div>
                                );
